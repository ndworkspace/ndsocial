...
/**
 * add by tangtaotao note: remember to user setOnPageSelectedListener instead of setOnPageChangeListener
 * because PageChangeListener is specified in the ImageViewTouchViewPager
 */
				...
				if (state == SCROLL_STATE_SETTLING
					...
					try {
						ImageViewTouch imageViewTouch = (ImageViewTouch) findViewWithTag(VIEW_PAGER_OBJECT_TAG
								+ previousPosition/*getCurrentItem()*/);//tangtaotao change from getCurrentItem() to previousPosition
						if (imageViewTouch != null) {
							imageViewTouch.zoomTo(1f, 200);//300
						} else { //tangtaotao add
							ImageViewTouch image = (ImageViewTouch) findViewWithTag(VIEW_PAGER_OBJECT_TAG
									+ getCurrentItem());
							if(image != null) {
								image.zoomTo(1f, 100);//300
							}
						}
