...
public class AnimateViewPager extends ViewPager {
	...
	public AnimateViewPager(Context context, AttributeSet attrs) {
		...
		TypedArray ta = context.obtainStyledAttributes(attrs, R.styleable.AnimateViewPager);
		int effect = ta.getInt(R.styleable.AnimateViewPager_style, 0);
		...
		setFadeEnabled(ta.getBoolean(R.styleable.AnimateViewPager_fadeEnabled, false));
		setOutlineEnabled(ta.getBoolean(R.styleable.AnimateViewPager_outlineEnabled, false));
		setOutlineColor(ta.getColor(R.styleable.AnimateViewPager_outlineColor, Color.WHITE));
		switch (mEffect) {
		case Stack:
		case ZoomOut:
			setFadeEnabled(true);
...
//		mRight = getChildAt(position+1);
	...
	public View findViewFromObject(int position) {
		//tangtaotao add start; for InfiniteViewPager
		PagerAdapter adapter = getAdapter();		
		if(adapter != null && (adapter instanceof InfinitePagerAdapter)) {			
			position = position % ((InfinitePagerAdapter)adapter).getRealCount();
		}
		//tangtaotao add end
