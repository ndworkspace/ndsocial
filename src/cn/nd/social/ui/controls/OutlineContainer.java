...
public class OutlineContainer extends FrameLayout implements Animatable {
	...
	private void init() {
		...
		mOutlinePaint.setStrokeWidth(UnitConverter.dpToPx(getResources(), 2));
		mOutlinePaint.setColor(getResources().getColor(R.color.holo_blue));
		mOutlinePaint.setStyle(Style.STROKE);

		int padding = UnitConverter.dpToPx(getResources(), 10);
	...
	protected void dispatchDraw(Canvas canvas) {
		...
		int offset = UnitConverter.dpToPx(getResources(), 5);
		if (mOutlinePaint.getColor() != AnimateViewPager.sOutlineColor) {
			mOutlinePaint.setColor(AnimateViewPager.sOutlineColor);
		}
