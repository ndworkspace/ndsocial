...
public class WifiNetProtocol {
	
	NetworkServerThread mNetworkThread;
	private final static int SERVER_PORT = 6000;
	private static WeakReference<WifiNetProtocol> sNetworkProtocol;
	public WifiNetProtocol(Handler handler,boolean isHotspot) {
		mNetworkThread = new NetworkServerThread(isHotspot);
		mNetworkThread.init(handler, SERVER_PORT);
		mNetworkThread.start();
		sNetworkProtocol = new WeakReference<WifiNetProtocol>(this);
	}
	
	public static WifiNetProtocol getWifiNetProtocol() {
